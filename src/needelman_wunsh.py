...
def load_fasta(file):
    ...
    content = ''.join(['<BREAK>' if line.startswith('>')
                       else line for line in lines])
    content = list(filter(None, content.split('<BREAK>')))
...
def DFS(dna1, dna2, config, V_table):
    n1 = len(dna1)
    n2 = len(dna2)

    stack = [(np.array([n1, n2]), np.array([-1, -1]))]
    path = []
    final_paths = []
...
def visualise_generate_spaces(dna, path, ind):
    ...
    s = [' ' for _ in path[:-1]]
    for dna_id, s_id in enumerate(tmp):
        s[s_id] = dna[dna_id]
    return ''.join(s)
...
if __name__ == "__main__":
    ...
    labels2, content2 = load_fasta(args.seq2)
    ...
    paths = DFS(dna1, dna2, config, table)
    visualise(dna1, dna2, paths)
